...
class Program
{
    static void Main()
    {
        ...
        // Obtenemos los datos desglosados
        var partidosTemp = (from linea in datos
                            let p = linea.Split(';')
                            select new
                            {
                                Equipo1 = p[0],
        ...
        // Equipos únicos
        var equipos = (from p in partidosTemp
                       from e in new[] { p.Equipo1, p.Equipo2 }
                       select e)
                      .Distinct()
        ...
        var competiciones = (from p in partidosTemp
                             select p.Competicion)
                            .Distinct()
                            .Select((nombre, i) => new Competicion { Id = i + 1, Nombre = nombre })
                            .ToList();

        // Partidos normalizados
        var partidos = (from p in partidosTemp
                        select new Partido
                        {
                            Id = partidosTemp.IndexOf(p) + 1,
